...
public class FloatingActionButtonWithText extends LinearLayout {
    ...
    public FloatingActionButtonWithText(Context context) {
        this(context, null);
    }
    ...
    public FloatingActionButtonWithText(Context context, AttributeSet attrs, int defStyle) {
    ...
    private void initAttributes(Context context, AttributeSet attributeSet) {
        TypedArray attr = context.obtainStyledAttributes(attributeSet, R.styleable.FloatingActionButtonWithText, 0, 0);
        if (attr != null) {
            try {
                mColorNormal = attr.getColor(R.styleable.FloatingActionButtonWithText_fabt_colorNormal, getColor(android.R.color.holo_blue_dark));
                mColorPressed = attr.getColor(R.styleable.FloatingActionButtonWithText_fabt_colorPressed, getColor(android.R.color.holo_blue_light));
                mSize = attr.getInt(R.styleable.FloatingActionButtonWithText_fabt_size, SIZE_NORMAL);
                mIcon = attr.getResourceId(R.styleable.FloatingActionButtonWithText_fabt_icon, 0);
            } finally {
                attr.recycle();
            }
        }
    ...
    private Drawable createCircleDrawable(RectF circleRect, int color) {
        final RectF textRect = new RectF(0, 0, 0 + 30, 0 + 20);
        final Bitmap bitmap = Bitmap.createBitmap(mDrawableSize*2, mDrawableSize*2, Config.ARGB_8888);
        final Canvas canvas = new Canvas(bitmap);

        final Paint paintIcon = new Paint();
        paintIcon.setAntiAlias(true);
        paintIcon.setColor(color);
        canvas.drawRoundRect(circleRect,4,4,paintIcon);
       // canvas.drawOval(circleRect, paintIcon);
